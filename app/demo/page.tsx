...
export default function DemoPage() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0d1117', display:'flex', flexDirection:'column', alignItems:'center', padding:'48px 24px' }}>
      ...
      <div style={{ textAlign:'center', marginBottom:48 }}>
        ...
        <h1 style={{ fontSize:28, fontWeight:700, marginBottom:8, letterSpacing:'-0.5px' }}>Interactive Demo</h1>
        <p style={{ fontSize:15, color:'#8b949e', maxWidth:480, lineHeight:1.6 }}>
          Explore ITFlow as different roles. Each account uses the same password —
          <strong style={{ color:'#e6edf3' }}> Demo1234!</strong> — with a fully pre-loaded
          environment across 8 sites.
        </p>
        <div style={{ display:'inline-flex', alignItems:'center', gap:8, marginTop:16, padding:'8px 16px', background:'rgba(245,166,35,0.08)', border:'1px solid rgba(245,166,35,0.2)', borderRadius:8, fontSize:13, color:'#F5A623' }}>
          ⊞ You are in the <strong>Demo environment</strong> — no real data affected
        </div>
        {error && (
          <div style={{ marginTop:12, padding:'8px 16px', background:'rgba(255,107,43,0.1)', border:'1px solid rgba(255,107,43,0.2)', borderRadius:8, fontSize:13, color:'#FF6B2B' }}>
            {error}
          </div>
        )}
      </div>
      ...
      {/* Role cards */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(260px, 1fr))', gap:16, width:'100%', maxWidth:1100, marginBottom:48 }}>
        {DEMO_ACCOUNTS.map(account => {
          ...
          return (
            <div key={account.role} style={{
              background:'#161b22',
              border:`1px solid ${isLoading ? account.color + '60' : 'rgba(255,255,255,0.08)'}`,
              borderRadius:14, padding:'24px',
              display:'flex', flexDirection:'column', gap:16,
              transition:'border-color 0.2s',
            }}>
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:44, height:44, borderRadius:'50%', background:`${account.color}22`, border:`2px solid ${account.color}44`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, fontWeight:700, color:account.color, flexShrink:0 }}>
                  {account.avatar}
                </div>
                <div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                ...
                <span style={{ fontSize:11, fontWeight:700, padding:'2px 7px', borderRadius:4, background:`${account.color}18`, color:account.color }}>
                  {account.siteCode} — {account.site}
                </span>
              </div>

              <p style={{ fontSize:13, color:'#8b949e', lineHeight:1.5, margin:0 }}>{account.desc}</p>

              <div style={{ flex:1 }}>
                {perms.map((p,i) => (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:7, padding:'4px 0', fontSize:12, color:'#8b949e' }}>
                    <span style={{ color:account.color, fontSize:10 }}>✓</span>{p}
                  </div>
                ))}
              </div>

              <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:8, padding:'10px 12px', fontSize:12 }}>
      ...
      {/* Site overview */}
      <div style={{ width:'100%', maxWidth:1100, background:'#161b22', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:'24px', marginBottom:32 }}>
        <div style={{ fontSize:13, fontWeight:600, marginBottom:16 }}>Demo environment — 8 sites pre-loaded</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:10 }}>
          {[
            { code:'ATX', name:'Austin HQ',    assets:312, color:'#2ABFA0', flag:''        },
            ...
            <div key={s.code} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:8, padding:'10px 14px', display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ width:32, height:32, borderRadius:6, flexShrink:0, background:`${s.color}18`, border:`1px solid ${s.color}40`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color:s.color }}>
                {s.code}
              </div>
