...
export function UsersTab() {
  ...
  function changeRole(id: string, role: AdminUser['role']) {
    ...
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button style={addBtn}>+ Invite user</button>
        </div>
      </div>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
          <thead>
            <tr style={{ borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
              {['User','Site','Role','Orgs','Last login','Status',''].map(h => (
          ...
          <tbody>
            {filtered.map(u => (
              <tr key={u.id} style={{ borderTop:'1px solid rgba(255,255,255,0.05)' }}>
                ...
                <td style={tdSt}>
                  <span style={{ background:'rgba(255,255,255,0.06)', padding:'2px 7px', borderRadius:4, fontSize:11, fontWeight:600 }}>
                    {u.siteCode}
                  </span>
                </td>
                <td style={tdSt}>
                  <select
                    ...
                    style={{
                      ...
                      fontWeight:600, color:ROLE_COLORS[u.role], cursor:'pointer', outline:'none',
                    }}
                  >
                  ...
                  <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                    {u.orgs.map(o => (
                      <span key={o} style={{
                        ...
                        background: o === 'Demo' ? 'rgba(245,166,35,0.12)' : 'rgba(42,191,160,0.12)',
                        color: o === 'Demo' ? '#F5A623' : '#2ABFA0',
                      }}>{o}</span>
                    ))}
                  </div>
                </td>
                ...
                <td style={tdSt}>
                  <span style={{
                    ...
                    background: u.active ? 'rgba(57,211,83,0.12)' : 'rgba(139,148,158,0.12)',
                    color: u.active ? '#39D353' : '#8b949e',
                  }}>
                ...
                <td style={{ ...tdSt, textAlign:'right' }}>
                  <button onClick={() => toggleActive(u.id)} style={{
                    ...
                    borderRadius:5, padding:'3px 10px', fontSize:11, color:'#8b949e', cursor:'pointer',
                  }}>
