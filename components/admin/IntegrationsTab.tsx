...
export function IntegrationsTab({ canManage = true }: { canManage?: boolean }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      {!canManage && <div style={{ marginBottom:14, padding:'9px 14px', background:'rgba(59,139,250,0.06)', border:'1px solid rgba(59,139,250,0.15)', borderRadius:8, fontSize:12, color:'#3B8BFA', display:'flex', alignItems:'center', gap:8 }}>🔒 Integration settings are Admin-only — contact your Admin to configure connections</div>}
                ...
                {integ.fields.map(f => (
                  <div key={f.key}>
                    ...
                    <input
                      ...
                      value={cfg[f.key] ?? ''}
                      onChange={e => canManage && setField(integ.id, f.key, e.target.value)}
                      placeholder={!canManage ? '••••••••' : f.placeholder}
                      disabled={!canManage}
                      style={{ width:'100%', background:'#0d1117', border:'1px solid rgba(255,255,255,0.1)', borderRadius:7, padding:'8px 12px', fontSize:13, color:'#e6edf3', outline:'none', opacity: !canManage ? 0.5 : 1 }}
                    />
                  </div>
                ))}
              </div>
